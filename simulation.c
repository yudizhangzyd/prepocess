...
void call_art(simu_options *opt, char *fq_out, char *fq_in) {
	if (opt->error_file2) {
		unsigned int cmd_len = strlen(opt->ART_command) + strlen(opt->error_file1) + strlen(opt->error_file2) + strlen(" -l -1 -2 -f -o -i -p -m 200 -s 10") + strlen(fq_out) + strlen(fq_in) + (int)(log10(opt->coverage) + 1) + (int)(log10(opt->length) + 1) + 8;
		mmessage(INFO_MSG, NO_ERROR, "Length of command: %u\n", cmd_len);
		char *command = malloc(cmd_len * sizeof *command);
		sprintf(command, "%s -1 %s -2 %s -l %d -i %s -f %d -o %s -p -m 200 -s 10",
